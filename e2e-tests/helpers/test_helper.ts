...
export class PageObject {
  ...
  async snapshotMessages({
    ...
    if (replaceDumpPath) {
      // Update page so that "[[crea-dump-path=*]]" is replaced with a placeholder path
  ...
  async snapshotServerDump(
    ...
    const dumpPaths = dumpPathMatches
      .map((match) => {
        const pathMatch = match.match(/\[\[crea-dump-path=([^\]]+)\]\]/);
    ...
    ).replaceAll(/\[\[crea-dump-path=([^\]]+)\]\]/g, "[[crea-dump-path=*]]");
  ...
  getAppPath({ appName }: { appName: string }) {
    return path.join(this.userDataDir, "crea-apps", appName);
  }
  ...
  async goToAppsTab() {
    ...
    await expect(this.page.getByText("Turn imagination into reality with Crea")).toBeVisible();
  }
...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp }, use) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$creaUserDataDir,
      });
      await use(po);
    },
    { auto: true },
  ...
  electronApp: [
    async ({ electronConfig }, use) => {
      ...
      process.env.SCALIX_ENGINE_URL = "http://localhost:3500/engine/v1";
      process.env.SCALIX_GATEWAY_URL = "http://localhost:3500/gateway/v1";
      process.env.E2E_TEST_BUILD = "true";
      // This is just a hack to avoid the AI setup screen.
      process.env.OPENAI_API_KEY = "sk-test";
      const baseTmpDir = os.tmpdir();
      const userDataDir = path.join(baseTmpDir, `crea-e2e-tests-${Date.now()}`);
      ...
      (electronApp as any).$creaUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          console.log("[cleanup:start] Killing crea.exe");
          console.time("taskkill");
          execSync("taskkill /f /t /im crea.exe");
          console.timeEnd("taskkill");
          console.log("[cleanup:end] Killed crea.exe");
        } catch (error) {
          console.warn(
            "Failed to kill crea.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ...
        : message.content
            ...
            .replace(
              /\n<crea-file path="package\.json">[\s\S]*?<\/crea-file>\n/g,
              "",
            );
      return `===\nrole: ${message.role}\nmessage: ${content}`;
    })
