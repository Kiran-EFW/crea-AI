(() => {
  const OVERLAY_ID = "__crea_overlay__";
  let overlay, label;
  ...
  function onClick(e) {
    ...
    window.parent.postMessage(
      {
        type: "scalix-component-selected",
        id: state.element.dataset.scalixId,
        name: state.element.dataset.scalixName,
      },
      "*",
    );
  }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "activate-scalix-component-selector") activate();
    if (e.data.type === "deactivate-scalix-component-selector") deactivate();
  });
  ...
  function initializeComponentSelector() {
    ...
    setTimeout(() => {
      if (document.body.querySelector("[data-scalix-id]")) {
        window.parent.postMessage(
          {
            type: "scalix-component-selector-initialized",
          },
          "*",
        );
