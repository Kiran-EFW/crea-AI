...
<crea-write path="new/image/file.png" description="Uploaded image to codebase">
DYAD_ATTACHMENT_0
</crea-write>
`;
      messageContent += "\n\n" + generateDump(req);
    ...
    if (
      ...
      messageContent = `
<crea-write path="src/bad-file.ts" description="Fix 2 errors and introduce a new error.">
...
x.nonExistentMethod2();
    ...
    if (
      ...
      // Fix errors in create-ts-errors.md and introduce a new error
      messageContent = `
<crea-write path="src/bad-file.ts" description="Fix remaining error.">
// Import doesn't exist
// import NonExistentClass from 'non-existent-class';
...
export default Index;
    ...
    if (
      ...
      messageContent = `[[STRING_IS_FINISHED]]";</crea-write>\nFinished writing file.`;
      messageContent += "\n\n" + generateDump(req);
    }
...
function generateDump(req: Request) {
  ...
  try {
    ...
    return `[[crea-dump-path=${dumpFilePath}]]`;
  } catch (error) {
