  ...
  if (scalixApiKey && settings.enableScalixPro) {
    ...
    if (providerConfig.gatewayPrefix != null || scalixEngineUrl) {
      ...
      const provider = isEngineEnabled
        ? createScalixEngine({
            apiKey: scalixApiKey,
            baseURL: scalixEngineUrl ?? "https://engine.scalix.world/v1",
      ...
      logger.warn(
        `Scalix Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
