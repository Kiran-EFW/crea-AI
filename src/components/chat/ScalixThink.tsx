...
import { VanillaMarkdownParser } from "./ScalixMarkdownParser";
import { CustomTagState } from "./stateTypes";
import { ScalixTokenSavings } from "./ScalixTokenSavings";

interface ScalixThinkProps {
  node?: any;
  children?: React.ReactNode;
}

export const ScalixThink: React.FC<ScalixThinkProps> = ({ children, node }) => {
  ...
  const tokenSavingsMatch =
    typeof children === "string"
      ? children.match(
          /^scalix-token-savings\?original-tokens=([0-9.]+)&smart-context-tokens=([0-9.]+)$/,
        )
      : null;

  // If it's token savings format, render CreaTokenSavings component
